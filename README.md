...
│
├── data/
│   ├── raw/                 # Raw temporal data (generated by fetch_temporal_data.py)
│   └── processed/           # Cleaned CSVs (openalex_nodes.csv, openalex_edges.csv)
│
├── src/                     
│   ├── 🟢 ACTIVE PILOT PIPELINE (Phase 0.5)
│   ├── openalex_cluster.py  # 1. API ingestion, graph building, Louvain clustering, CSV export
│   ├── measure_pressure.py  # 2. Null model & Z-score calculation (Epistemic Readiness)
│   ├── inspect_plates.py    # 3. Community inspection & semantic sampling
│   ├── export_for_gephi.py  # 4. GEXF export for topological visualization
│   │
│   ├── 🟡 AUDIT & FUTURE PHASES (Phase 1+)
│   ├── openalex_audit.py    # Reality Audit: Validates data density and topology health
│   ├── fetch_temporal_data.py # Phase 1 Prep: Fetches 2010-2016 data for temporal tracking
│   │
│   ├── ⚪ LEGACY / DEV SCRIPTS (Functionality absorbed by openalex_cluster.py)
│   ├── build_graph.py       # Parses local pilot_search_raw.csv into clean CSVs
│   ├── cluster_graph.py     # Clusters pilot CSVs
│   └── cluster_csv.py       # Clusters OpenAlex CSVs and exports GEXF
│
├── outputs/             
